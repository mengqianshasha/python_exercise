...
class Snake:
    ...
    def hit_himself(self):
        for node in self.body[:-1]:
            if node.x == self.currentX and node.y == self.currentY:
                return True
        return False
...
class Game:
    ...
    def eat_food(self):
        if self.snake.currentX == self.food.x and self.snake.currentY == self.food.y:
            self.food.update_location(round(random.randrange(0, self.dis_width - self.block_size) / 10.0) * 10.0, round(random.randrange(0, self.dis_height - self.block_size) / 10.0) * 10.0)
            self.score += 1 + round((self.move_speed - 3) * 0.5)
            self.snake.length += 1

    def snake_hit_wall(self):
        return self.snake.currentX >= self.dis_width or self.snake.currentX < 0 \
               or self.snake.currentY >= self.dis_height or self.snake.currentY < 0
